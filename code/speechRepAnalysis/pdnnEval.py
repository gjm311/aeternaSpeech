...
def featAgg(mod,rep,spk_path,ef):
...
if __name__=="__main__":
    ...
    spkDict,pdNames,hcNames=featAgg(mod,rep,spk_path,ef)
    num_pd=len(pdNames)
    num_hc=len(hcNames)
    spks=pdNames+hcNames
    num_spks=len(spks)
...
#         lr_scores.to_csv(save_path+mod+'_'+rep+"lrResults.csv")
    ...
    if rep_typ=='mc_fuse':
        trainResults.to_pickle(save_path+mod+'_mcFusion_trainResults.pkl')
        testResults.to_pickle(save_path+mod+'_mcFusion_testResults.pkl')
    if rep_typ in ['broadband','narrowband','wvlt']:
