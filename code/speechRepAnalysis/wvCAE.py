...
#         self.conv4=nn.Conv2d(8, 16, kernel_size=3, stride=1, padding=1, bias=False)
#         self.bn4 = nn.BatchNorm2d(16)

        self.linear = nn.Linear(32*8*2, dim)
...
class CAEdec(nn.Module):
    def __init__(self, dim=256, nc=1):
        super().__init__()
        self.conv1=nn.ConvTranspose2d(32, 16, kernel_size=3, stride=1, padding=(1,0), bias=False)
        self.bn1 = nn.BatchNorm2d(16)
        self.conv2=nn.ConvTranspose2d(16,8, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn2 = nn.BatchNorm2d(8)
#         self.conv3=nn.ConvTranspose1d(2, 1, kernel_size=3, stride=1, padding=1, bias=False)
#         self.bn3 = nn.BatchNorm2d(1)
#         self.conv4=nn.ConvTranspose2d(8, nc, kernel_size=3, stride=1, padding=1, bias=False)
        self.linear = nn.Linear(dim,32*8*2)

    def forward(self, x):
            
        x = self.linear(x)
        x = x.view(x.size(0), 32, 8, 2)
